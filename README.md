...
### 面评
To 写下这段话的面试官 - 我从事前端开发近7年（十万级~亿级 toC 产品），就因为我写了几个月爬虫相关项目，就得出这种结论，你不觉得你的面评很搞笑吗？

我们面试中有关爬虫的内容一点儿没聊，你一定要用这种低级的方式证明我不适合这个职位吗？

<img width="320" alt="image" src="https://github.com/user-attachments/assets/96a20b03-fce4-46c2-a8ac-e9c032418944" />

### 面试方式
To 一些需要现场面试的公司 - 我1小时地铁吭呲吭呲赶到贵司，和贵司面试官聊天，20分钟面试结束把我打发走，我又1小时吭呲吭呲回家。你邀约我面试的目是什么？完成到面率KPI？非要浪费大家的时间吗？

### 流程推进
To 我所遇到的大大小小的公司 - 能不能少一些横向对比？一个流程20天没反馈，HR 经常不接电话玩 Ghosting；请问，这到底是在干什么？就不能有个痛快话吗？

### 面试官的刻板印象
#### “你既然写出了这个项目，那你是不是很好找工作啊？”
对不起，不是；既然你说我好找工作，那给我Offer啊

#### “你写出了这个项目，同时我也看到你几份工作时间不长；那你是不是不稳定啊？难道你来了以后天天就想着跑？”
近期离职原因：部门解散、业务HC缩减

你以为我真的想不稳定吗？难道我入职就是为了离职而不是在劳动岗位上创造价值？
...
#### 不期望投递公司 / 职位关键词（支持排除词与精准匹配）

不想再看到某些公司、某些类型的职位？把关键词填进去，程序会在开聊前把它们挑出来：

- **不期望投递公司**：公司名称按所选的匹配模式命中任一关键词即视为不期望投递，例如 `外包,劳务派遣,某某科技`
- **不期望投递职位**：职位名称 / 职位类型 / 职位描述中命中任一关键词即视为不期望投递，匹配范围（勾选这三项中的哪几项）可自定义，例如 `外包,驻场,销售,实习`
- **排除词**：命中屏蔽词、但同时命中排除词时会被**放行**，用来消解误伤。例如屏蔽 `外包`、把 `非外包` 填进排除词，“非外包”的职位就不会被跳过
- **匹配模式**：公司、职位各自可选
  - **包含匹配**（默认）：文本里只要出现关键词就算命中，最宽松
  - **精准匹配**：先去掉空白，再按标点（`、/（）-` 等）与中英文交界处切成词，关键词要与**某个词完全一致**才算命中。填 `前端` 就不会再命中 `资深前端`、`前端开发工程师`，但 `前端 / 后端`、`Java开发` 里的 `前端`、`Java` 仍然会命中；注意英文短语会被视作一个整体（`Senior Front End Developer` 切不出 `front`），需要宽松匹配请用包含匹配
  - 排除词与屏蔽词**共用同一个模式**，不存在两套规则
- 关键词**逗号分隔**，不区分大小写、忽略空格；输入框留空表示不按关键词筛选
- 公司、职位各自可设置“命中后如何处理”：**在 BOSS 直聘上标记不合适**（推荐，可让 BOSS 直聘推荐新职位置换）、**在本地数据库中记录且 7 天内跳过**、**仅在本次运行中跳过**
- 处理方式选“仅在本次运行中记录不合适”时，命中职位名称的职位会在**职位列表阶段**就被跳过，不点开详情，速度最快
- 关键词、匹配范围、匹配模式、排除词都可以放到「设置 - 公共职位筛选条件」里，供多份配置复用；从旧版本升级的用户，原先的“不期望投递公司正则”会自动转换为关键词，无需重新配置
- 命中与放行都会打印日志（`[屏蔽职位] …命中关键词“…”` / `…被排除词“…”放行`；非默认匹配模式会额外标注 `匹配模式：精准匹配`），方便排查误伤
