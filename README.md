...
- **不期望投递公司**：公司名称包含任一关键词即视为不期望投递，例如 `外包,劳务派遣,某某科技`
- **不期望投递职位**：职位名称 / 职位类型 / 职位描述中包含任一关键词即视为不期望投递，匹配范围（勾选这三项中的哪几项）可自定义，例如 `外包,驻场,销售,实习`
...
- 关键词、匹配范围、排除词都可以放到「设置 - 公共职位筛选条件」里，供多份配置复用；从旧版本升级的用户，原先的“不期望投递公司正则”会自动转换为关键词，无需重新配置
- 命中与放行都会打印日志（`[屏蔽职位] …命中关键词“…”` / `…被排除词“…”放行`），方便排查误伤
